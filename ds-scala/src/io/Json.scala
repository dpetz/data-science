...
import io.Parser.{Cons, Or, Parser, Rep, Rex, Then, Trans, Gate, Pre, Post}
...
object Json {
  ...
  val pJson = Gate(spaces ,pNum | pStr | pNull | pTrue | pFalse | pArr, spaces)

  val spaces = Rep(Rex(1)("""\s"""))


  val pArr:Parser[Arr] = Gate("[",Then(pJson,Rep(Pre(",",pJson))),"]") > { Arr(_) }
  ...
  object Num {  def apply(s:String)=s.toDouble }
